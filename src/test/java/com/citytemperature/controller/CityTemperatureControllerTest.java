...
class CityTemperatureControllerTest {
    ...
    @MockBean
    private CityTemperatureService cityTemperatureServiceMock;
    ...
    @Test
    void shouldReturn400WhenUserInputAnInvalidCityNameAndResponseBodyShouldHaveAClearMessageAboutTheErrorInDetailsKey() throws Exception {
    ...
    void shouldReturn404WhenServiceThrowsCityNotFoundExceptionAndResponseBodyShouldHaveTheMessageOfTheException() throws Exception {
        final String message = "Clear message about the error";
        when(this.cityTemperatureServiceMock.findCityTemperature(any())).thenThrow(new CityNotFoundException(message));
        this.mockMvc
                .perform(getDefaultRequest())
    ...
    @Test
    void shouldReturn404WhenServiceThrowsCityTemperatureNotFoundExceptionAndResponseBodyShouldHaveTheMessageOfTheException() throws Exception {
        final String message = "Another clear message about the error";
        when(this.cityTemperatureServiceMock.findCityTemperature(any())).thenThrow(new CityTemperatureNotFoundException(message));
        this.mockMvc
                .perform(getDefaultRequest())
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.details").value(message));
    ...
    @Test
    void shouldReturn500WhenServiceThrowsAnUnexpectedExceptionAndResponseBodyShouldHaveADefaultMessageIgnoringExceptionMessage() throws Exception {
        final String expectedBody = "{'details': 'An error has occurred and your team is working on it. Try again later.'}";
        when(this.cityTemperatureServiceMock.findCityTemperature(any())).thenThrow(new RuntimeException("Ignore me."));
        this.mockMvc
                .perform(getDefaultRequest())
